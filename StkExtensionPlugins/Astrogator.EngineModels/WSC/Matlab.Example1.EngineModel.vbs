...
m_mFilename = "example1EngineModel"
...
Function PrePropagate( AgGatorPluginResultState )
	...
	If( Not AgGatorPluginResultState is Nothing) then
		...
		Call m_MatlabApp.PutWorkspaceData("engMdlT0", "global", CDbl(m_T0))
		Call m_MatlabApp.PutWorkspaceData("engMdlT1", "global", CDbl(m_T1))
		Call m_MatlabApp.PutWorkspaceData("engMdlT2", "global", CDbl(m_T2))
		Call m_MatlabApp.PutWorkspaceData("engMdlTS", "global", CDbl(m_Ts))
		Call m_MatlabApp.PutWorkspaceData("engMdlTC", "global", CDbl(m_Tc))
...
Function Evaluate( AgGatorPluginResultEvalEngineModel )
	...
	If( Not AgGatorPluginResultEvalEngineModel is Nothing) then
			...
			Call m_MatlabApp.PutWorkspaceData("engMdlDeltaT", "base", CDbl(deltaT))
			Call m_MatlabApp.PutWorkspaceData("engMdlArgOfLat", "base", CDbl(argOfLat))		

			' Execute the mfile on those variables
			
			Dim outResult

			Call m_MatlabApp.Feval(m_mFilename, 1, outResult, "engMdlDeltaT=", "engMdlArgOfLat=")

			' Get the computed variables		

			Thrust = CDbl(outResult(0))	
