...
Ephemeris File Reader Plugin Samples components.
...
3.  This step registers your component as a COM component.
    ...
    'regsvr32 YOUR_FILENAME'. For example: regsvr32 VBScript.Example.EphemerisFileReader.wsc  .

    Dll components not built using .NET

    Create a command window (under the Start menu, hit "Run ...", type 'Command' and hit OK).
    Change your directory (using cd) to that of the file. At the command prompt, type 
    'regsvr32 YOUR_FILENAME'. For example: regsvr32 AGI.Astrogator.Plugin.Examples.EngineModeling.CPP.Example1 .
    ...
    regasm /codebase AGI.EphemerisFileReader.Plugin.Examples.CSharp.Examples.dll
...
4.	This plugin has already been registered in the "EphemerisFileReader" Category in the
	"EphemerisFileReader.xml" plugin point registration file; however the registration lies
...
7.  Use STK documentation for directions on how to configure an Engine Model Plugin
