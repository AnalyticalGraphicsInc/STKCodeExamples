...
#-----------------------------------------------------------------------------------
#
# The atmospheric reflectance being modeled is that of of sphere, 
# exactly like the Spherical model does
#
#-----------------------------------------------------------------------------------
...
my $dirpath = 'C:\Temp';
my $m_DebugFile = $dirpath.'\DragSpherical_Debug.txt';
...
sub Evaluate
{
	...
	if($m_Enabled == true && defined($Result) )
	{
		...
		{
			$m_Enabled = setSphericalReflectanceInBody($Result, $cd);
		}
		else
		{
			$m_Enabled = setSphericalReflectanceUsingFrame($Result, $cd, $m_RefFrame);
		}
	}
...
sub setSphericalReflectanceInBody
{
	...
	if(defined($incidentVecArrayRef))
	{
		...
		my $x = $m_DragArea * $incidentVecArrayRef->[0];
		my $y = $m_DragArea * $incidentVecArrayRef->[1];
		my $z = $m_DragArea * $incidentVecArrayRef->[2];
		...
		if(defined($incidentDirPosPartialsArrayRef))
		{
			...
			for ($k=0; $k<9; $k++)
			{
				$posParts[$k] = $BCoeff * $incidentDirPosPartialsArrayRef->[$k] ;
			}
			...
			if(defined($incidentDirVelPartialsArrayRef))
			{
				...
				for ($k=0; $k<9; $k++)
				{
					$velParts[$k] = $BCoeff * $incidentDirVelPartialsArrayRef->[$k] ;
				}
...
sub setSphericalReflectanceUsingFrame
{
	...
	if(defined($incidentVecArrayRef))
	{
		...
		{
		    #printf DEBUGFILE "CD partials: %20.15e, %20.15e, %20.15e\n", $x, $y, $z;
			$Result->SetReflectanceParamPartials($m_CDIndex, $frame, $x, $y, $z);
		}
		...
		if(defined($incidentDirPosPartialsArrayRef))
		{
			#writeMatrix($incidentDirPosPartialsArrayRef, "IncInrtPPs");
			...
			#writeMatrix(\@posParts, "OutInrtPPs");
			...
			if(defined($incidentDirVelPartialsArrayRef))
			{
				#writeMatrix($incidentDirVelPartialsArrayRef, "IncInrtVPs");
				...
				#writeMatrix(\@velParts, "OutInrtVPs");
			}

			my $tempStr = "$m_MsgCntr> $cd : frame = $frame : ($x, $y, $z)";
			DebugMsg($tempStr);
