...
Search Plugin Samples components.
...
3.  This step registers your component as a COM component.
    ...
    'regsvr32 YOUR_FILENAME'. For example: regsvr32 JScript.SearchExample.WSC  .

    Dll components not built using .NET

    Create a command window (under the Start menu, hit "Run ...", type 'Command' and hit OK).
    Change your directory (using cd) to that of the file. At the command prompt, type 
    'regsvr32 YOUR_FILENAME'. For example: regsvr32 AGI.Search.Plugin.Examples.CPP.BisectionPlugin.1 .
    ...
    regasm /codebase AGI.Search.Plugin.Examples.CSharp.BisectionPlugin.1  .
...
4.	This plugin has already been registered in the "Search" Category in the
	"Search.xml" plugin point registration file; however the registration lies
...
7.  Use STK documentation for directions on how to configure a Search Plugin.
