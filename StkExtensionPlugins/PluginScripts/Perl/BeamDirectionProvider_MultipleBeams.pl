##########################################################################################
# EXAMPLE PERL BASED SCRIPT "BEAM DIRECTION PROVIDER" PROVIDED BY THE USER
...
# Perl_BeamDirectionProvider_MultipleBeams

# declare some global variables

# my $Perl_BeamDirectionProvider_MultipleBeams_compute_init = -1;
my $Perl_BeamDirectionProvider_MultipleBeams_Inputs, $Perl_BeamDirectionProvider_MultipleBeams_Outputs;
my %Perl_BeamDirectionProvider_MultipleBeams_Outputs_ArgHash;
...
$gScanMinAz = -45*$degToRad;
$gScanMaxAz = 45*$degToRad;
$gScanMinEl = -55*$degToRad;
$gScanMaxEl = -5*$degToRad;

$gScanAz = $gScanMaxAz;
$gScanEl = $gScanMaxEl;

sub Perl_BeamDirectionProvider_MultipleBeams
{
    ...
    if ( !defined($inputData[0]) )
    {
        ...
        @retVal = Perl_BeamDirectionProvider_MultipleBeams_compute(@inputData);

    }
    elsif ( $inputData[0] eq 'register' )
    {
        $Perl_BeamDirectionProvider_MultipleBeams_compute_init = -1;

        @retVal = Perl_BeamDirectionProvider_MultipleBeams_register();
...
sub Perl_BeamDirectionProvider_MultipleBeams_register
{
    my @argStr;
...
sub Perl_BeamDirectionProvider_MultipleBeams_compute
{
    ...
    if($Perl_BeamDirectionProvider_MultipleBeams_compute_init < 0)
    {
        $Perl_BeamDirectionProvider_MultipleBeams_compute_init = 1; 
        ...
        $Perl_BeamDirectionProvider_MultipleBeams_Inputs = $g_PluginArrayInterfaceHash{'Perl_BeamDirectionProvider_MultipleBeams'}{'Inputs'};
        $Perl_BeamDirectionProvider_MultipleBeams_Outputs = $g_PluginArrayInterfaceHash{'Perl_BeamDirectionProvider_MultipleBeams'}{'Outputs'};

        %Perl_BeamDirectionProvider_MultipleBeams_Outputs_ArgHash = $Perl_BeamDirectionProvider_MultipleBeams_Outputs->getArgumentHash();

        # comment out the line below if you don't want to see the inputs and outputs each time the script is run
        #Perl_BeamDirectionProvider_MultipleBeams_showArgs();
    }
    ...
    my $objAz, $objEl, $objRange, $radarRange, $maxNumBeams, $minAz, $minEl, $minRange;
    ...
    if ($NumDirections < 1) 
    {
        ...
        if ($gScanAz < $gScanMinAz)
        {
            $gScanEl = $gScanEl - $gScanElStepSize;
            $gScanAz = $gScanMaxAz;
        }

        if ($gScanEl < $gScanMinEl) 
        {
            $gScanEl = $gScanMaxEl;
        }
...
sub Perl_BeamDirectionProvider_MultipleBeams_showArgs
{
    my @argStrArray;

    STKUtil::printOut "Doing Perl_BeamDirectionProvider_MultipleBeams_compute_init\n";

    @argStrArray = ();

    push @argStrArray, $Perl_BeamDirectionProvider_MultipleBeams_Inputs->{'FunctionName'}->{'Name'} . " Inputs \n";

    # the first arg on input is the calling mode

    push @argStrArray, "0 : this is the calling mode\n";

    my @args = $Perl_BeamDirectionProvider_MultipleBeams_Inputs->getArgumentArray();
    ...
    foreach $arg (@args)
    {
        ($index, $descrip) = $Perl_BeamDirectionProvider_MultipleBeams_Inputs->getArgument($arg);
    ...
    push @argStrArray, $Perl_BeamDirectionProvider_MultipleBeams_Outputs->{'FunctionName'}->{'Name'} . " Outputs \n";

    my @args = $Perl_BeamDirectionProvider_MultipleBeams_Outputs->getArgumentArray();
    ...
    foreach $arg (@args)
    {
        ($index, $descrip) = $Perl_BeamDirectionProvider_MultipleBeams_Outputs->getArgument($arg);
