...
'	range = Sqr((xmtrX-rcX)^2 + (xmtrY-rcY)^2 + (xmtrZ-rcZ)^2)
	...
	returnValue(VB_ReceiverModel_Outputs.Frequency)           = 12.3e9
	returnValue(VB_ReceiverModel_Outputs.Bandwidth)           = 25.0e6
	returnValue(VB_ReceiverModel_Outputs.Gain)                = 30
	returnValue(VB_ReceiverModel_Outputs.PreReceiveLoss)      = 2.0
	returnValue(VB_ReceiverModel_Outputs.PreDemodLoss)        = 0.0
	returnValue(VB_ReceiverModel_Outputs.UseRainModel)        = 0
	returnValue(VB_ReceiverModel_Outputs.RainOutagePercent)   = 0.3
