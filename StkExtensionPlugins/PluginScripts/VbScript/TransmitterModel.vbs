...
'	range = Sqr((xmtrX-rcX)^2 + (xmtrY-rcY)^2 + (xmtrZ-rcZ)^2)
	...
	returnValue(VB_TransmitterModel_Outputs.Frequency)        = 12.3e9
	returnValue(VB_TransmitterModel_Outputs.Power)            = 35
	returnValue(VB_TransmitterModel_Outputs.Gain)             = 30
	returnValue(VB_TransmitterModel_Outputs.DataRate)         = 12.0e6
	returnValue(VB_TransmitterModel_Outputs.Bandwidth)        = 25.0e6
	returnValue(VB_TransmitterModel_Outputs.Modulation)       = "BPSK"
	returnValue(VB_TransmitterModel_Outputs.PostTransmitLoss) = -2.3
