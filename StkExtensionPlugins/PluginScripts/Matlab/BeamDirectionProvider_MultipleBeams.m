function [output] = Matlab_BeamDirectionProvider_MultipleBeams(input)
...
% DO NOT CHANGE ANYTHING ELSE IN THE SCRIPT
	...
	gScanMinAz = -45*degToRadians; 
	gScanMaxAz = 45*degToRadians;    
	gScanMinEl = -55*degToRadians;   
	gScanMaxEl = -5*degToRadians;
	
	global gBdpMbScanAz;
	global gBdpMbScanEl;
	
	if isempty(gBdpMbScanAz) || isempty(gBdpMbScanEl)
    	gBdpMbScanAz = gScanMaxAz;
	    gBdpMbScanEl = gScanMaxEl;	
	end
...
switch input.method
	...
	%
	% This simple sample demonstrates how to dynamically return beam directions.  This example
	% assumes it is used as a ground radar with z-axis looking at horizon. 	This script 
	% defines the antenna's field of regard (FOR) and then scans the FOR.  If any aircraft fly   
	% within the FOR and within effective range it will switch to tracking mode.  It will 
	% track up to maxNumBeams objects (user should ensure maxNumBeams < numElements). If there 
	% are more than maxNumBeams of objects within radar range, it will only track the first 
    % maxNumBeam objects.  If all objects are out of the FOR, it will 
	...
	for i = 0:NumberOfMembers - 1   
	    ...
	    if objRange < radarRange
			...
			if objAz > -PiOver2
				if objAz < PiOver2
					Azimuths(NumDirections+1)   = objAz;
					Elevations(NumDirections+1) = objEl;
	...
	if NumDirections < 1
        gBdpMbScanAz = gBdpMbScanAz - gScanAzStepSize;
	    if EpochSec <= 0.00001		  
	      gBdpMbScanAz = gScanMaxAz;
	      gBdpMbScanEl = gScanMaxEl;
	    end
    	Azimuths(1) = gBdpMbScanAz;
	    Elevations(1) = gBdpMbScanEl;
		
        % Check for end of scan pattern and reset to begin scan pattern
		if gBdpMbScanAz < gScanMinAz
			gBdpMbScanEl = gBdpMbScanEl - gScanElStepSize;
			gBdpMbScanAz = gScanMaxAz;
		end
		
		if gBdpMbScanEl > gScanMinEl
			gBdpMbScanEl = gScanMaxEl;
		end
		NumDirections = 1;
    end
