...
namespace ReentryCalculator.Utility
{
    class STK
    {
        ...
        public static void ConfigurePropagator(AgStkObjectRoot root)
        {
            ...
            string nrlPath = Path.Combine(propagatorsPath, "NRLMSISE 2000.Propagator");
            string jcrPath = Path.Combine(propagatorsPath, "Jacchia-Roberts.Propagator");

            if (!File.Exists(nrlPath))
            {
                MessageBox.Show("did not find " + nrlPath);
                return;
            }

            if (!File.Exists(jcrPath))
            {
                MessageBox.Show("did not find " + jcrPath);
                return;
            }
