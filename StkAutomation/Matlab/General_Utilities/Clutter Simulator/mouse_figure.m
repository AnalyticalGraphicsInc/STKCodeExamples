...
%               practically the exact code ^_^
    ...
    function scroll_zoom(varargin)
        ...
        % Check to see if CurrentPoint is inbounds of the xlim and ylim
        if old_position(1,1) < xlim(1) || old_position(1,1) > xlim(2) || old_position(1,2) < ylim(1) || old_position(1,2) > ylim(2)
            disp('Out of Bounds!') ;
            return ;
        end
    ...
    function pan_click(varargin)
        ...
        % get the axes' x- and y-limits
        xlim = get(axs, 'xlim');  ylim = get(axs, 'ylim');
        old_position = get(axs, 'CurrentPoint');
        if old_position(1,1) < xlim(1) || old_position(1,1) > xlim(2) || old_position(1,2) < ylim(1) || old_position(1,2) > ylim(2)
            disp('Out of Bounds!') ;
            return ;
        end
