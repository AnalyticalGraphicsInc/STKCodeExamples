...
class configSettings:
    def __init__(self, fileName='LifeTimeAnalysis.csv',howToVary='LatinHyperCube',numCores=2,runHPOP=False,
                 maxDur = 100,decayAlt = 65,
                 epoch=20001,
                 a=6778,e=0,i=45,AoP=0,RAAN=0,TA=0,
                 Cd=2.2,Cr=1.0,DragArea=13.65,SunArea=15.43,Mass=1000,
                 AtmDen='Jacchia 1971',SolFlxFile='SolFlx_CSSI.dat',SigLvl=0,OrbPerCal=10,GaussQuad=2,SecondOrderOblateness='Off',
                 numberOfRuns=1,varyCols=[],varyValues=[],setSunAreaEqualToDragArea=True):
    ...
    def properties(self):
        for attr in dir(self):
            if not attr.startswith('__') and attr != 'properties' and attr != 'propertiesDict':
...
def generateTradeStudy(tradeStudy):
    ...
    Ra = a*(1+e)
    p = a*(1-e**2)
    rs,vs = coe2rv(GM_earth*1e-9, p, e, i*np.pi/180, RAAN*np.pi/180, AoP*np.pi/180, TA*np.pi/180)
    x = rs[0]
    y = rs[1]
    z = rs[2]
    Vx = vs[0]
    ...
    data = [epoch,a,e,i,RAAN,AoP,TA,Rp,Ra,p,
            x,y,z,Vx,Vy,Vz,
            Cd,Cr,DragArea,SunArea,Mass,CdAM,CrAM,
            OrbPerCal,GaussQuad,SigLvl,SolFlxFile,AtmDen,SecondOrderOblateness]

    columns = ['epoch','a', 'e', 'i', 'RAAN', 'AoP', 'TA', 'Rp', 'Ra', 'p',
               'x', 'y', 'z','Vx', 'Vy', 'Vz',
               'Cd', 'Cr', 'Drag Area', 'Sun Area', 'Mass','Cd*Drag Area/Mass', 'Cr*Sun Area/Mass',
    ...
    #     lhd = stats.norm(loc=0, scale=1).ppf(lhd) # Convert to a normal distribution
        ...
        strii = [ii for ii in range(len(varyValues)) if isinstance(varyValues[ii][0],str)]
        for ii in strii:
            lhd.iloc[:,ii] = pd.cut(lhd.iloc[:,ii],len(varyValues[ii]),labels=varyValues[ii])
        ...
        lhd.loc[:,floatii] = lhd.loc[:,floatii]*(lhsMinMax[:,1]-lhsMinMax[:,0])+lhsMinMax[:,0]

        indxs = [ii for ii in range(len(varyCols)) if varyCols[ii] in ['Orb Per Calc','Gaussian Quad','Flux Sigma Level']]
        ...
        if adjustEpoch == True: 
            df['epoch'] = [adjustDate(yyddd,deltaDay) for yyddd,deltaDay in zip(minDate,deltaDays)]
    ...
    # Perturb
        ...
        if 'epoch' in varyCols:
            df['epoch'] = [adjustDate(yyddd,deltaDay)for yyddd,deltaDay in zip(df['epoch'],pertdf['epoch'])]
            varyCols.remove('epoch')
            varyValues = varyValues[1:]
    ...
    cols = [col for col in df.columns if col not in ['SolarFluxFile','Density Model','2nd Order Oblateness']]
...
def lifetimeVariations(fileName,tradeStudy):
        ...
        dfAllTemp = pd.read_csv(fileName,index_col=0).reset_index(drop=True) # Read previously run Lifetime results
        df = pd.DataFrame()
        ...
        df.to_csv(os.getcwd()+'\\Results\\'+ tradeStudy.fileName) # Create a new csv to store the results  
...
def updateDf(df,runHPOP,varyCols,setSunAreaEqualToDragArea):
    ...
    if any(col in ['a','e','i','AoP','RAAN','TA','Rp','Ra'] for col in varyCols):
        ...
        if any(col in ['i','AoP','RAAN','TA'] for col in varyCols):
            df.loc[df['i']>180,'i'] = 180-(df.loc[df['i']>180,'i']-180) # if > 180, subtract the amount over from 180
       ...
       # Update Cartesian
        ...
        for ii in range(len(df)):    
            rs[ii],vs[ii] = coe2rv(GM_earth*1e-9, df['p'].iloc[ii], df['e'].iloc[ii], df['i'].iloc[ii]*np.pi/180, df['RAAN'].iloc[ii]*np.pi/180, df['AoP'].iloc[ii]*np.pi/180, df['TA'].iloc[ii]*np.pi/180)
        df['x'] = rs[:,0]
        df['y'] = rs[:,1]
        df['z'] = rs[:,2]
        df['Vx'] = vs[:,0]
    ...
    if any(col in ['x','y','z','Vx','Vy','Vz'] for col in varyCols):
        ...
        for ii in range(len(df)):  
            oes[ii,0], oes[ii,1], oes[ii,2], oes[ii,3], oes[ii,4], oes[ii,5] = rv2coe(GM_earth*1e-9, rs[ii], vs[ii])
        df['p'] = oes[:,0]
        df['e'] = oes[:,1]
...
def RunSTK(dfSlice,tradeStudy,processNum,showSTK=False,saveEveryNIter=15):
    ...
    fileNamekk = str.split(tradeStudy.fileName,'.')[0] + str(processNum) + '.' + str.split(tradeStudy.fileName,'.')[-1]
    ...
    def updateFlags(tradeStudy):
        ...
        if any(col in ['epoch','x','y','z','Vx','Vy','Vz','a','e','i','Aop','RAAN','TA','Rp','Ra'] for col in varyCols):
            updateState = True

        if any(col in ['Cd','Cr','Drag Area','Sun Area','Mass'] for col in varyCols):
            updateSatProp = True

        if any(col in ['Orb Per Calc','Gaussian Quad','Flux Sigma Level','SolarFluxFile','Density Model','2nd Order Oblateness'] for col in varyCols):
    ...
    if runHPOP == True:
        if len(df) > 0:
            for i in range(len(df)):
                if np.isnan(df['HPOP Years'][i]): # Not previously run
                    ...
                    prop = sat2.Propagator.QueryInterface(STKObjects.IAgVePropagatorHPOP)

                    # Initialize satellite
                    if updateEachEpoch == True:
                        epYr = root.ConversionUtility.ConvertDate('YYDDD','EpYr',dfstr['epoch'].iloc[i])
                    else:
                        epYr = epoch1
                        
                    prop.EphemerisInterval.SetExplicitInterval(float(epYr),float(epYr)+1e-5)
                    prop.InitialState.OrbitEpoch.SetExplicitTime(epYr)
                    prop.InitialState.Representation.AssignCartesian(STKUtil.eCoordinateSystemICRF,df['x'].iloc[i],df['y'].iloc[i],df['z'].iloc[i],df['Vx'].iloc[i],df['Vy'].iloc[i],df['Vz'].iloc[i])
                    prop.Propagate()
                    ...
                    if updateLTtradeStudy == True:
                        cmd = ('SetLifetime */Satellite/' + satName +
                       ' DensityModel "'+ dfstr['Density Model'].iloc[i] + '"' +
                       ' FluxSigmaLevel ' + dfstr['Flux Sigma Level'].iloc[i] +
                       ' OrbPerCalc ' + dfstr['Orb Per Calc'].iloc[i] +
                       ' GaussianQuad ' + dfstr['Gaussian Quad'].iloc[i] +
                       ' 2ndOrder ' + dfstr['2nd Order Oblateness'].iloc[i])
                        root.ExecuteCommand(cmd)
                        cmd = ('SetLifetime */Satellite/' + satName +
                        ' SolarFluxFile "C:\\ProgramData\\AGI\\STK 11 (x64)\\DynamicEarthData\\' + dfstr['SolarFluxFile'].iloc[i] + '"')
                        root.ExecuteCommand(cmd)
                    ...
                    if res.Item(0).split()[1] == 'decay':
                        ...
                        df.loc[i,'LT Years'] = float(res.Item(0).split()[7]) - float(epYr)
                    ...
                    prop.EphemerisInterval.SetExplicitInterval(float(epYr),float(epYr)+maxDur)
                    prop.Step = 60
                    prop.Integrator.ReportEphemOnFixedTimeStep = False
                    if dfstr['Density Model'].iloc[i] in ['NRLMSISE 2000','MSISE 1990']:
                        prop.Integrator.UseVOP = False
                    else:
                        prop.Integrator.UseVOP = True
                    drag = prop.ForceModel.Drag.DragModel.QueryInterface(STKObjects.IAgVeHPOPDragModelSpherical)
                    srp = prop.ForceModel.SolarRadiationPressure.SRPModel.Model.QueryInterface(STKObjects.IAgSRPModelSpherical)
                    prop.ForceModel.Drag.SetSolarFluxGeoMagType(STKObjects.eSolarFluxGeoMagUseFile)
                    dragFile = prop.ForceModel.Drag.SolarFluxGeoMag.QueryInterface(STKObjects.IAgVeSolarFluxGeoMagUseFile)
                    prop.ForceModel.MoreOptions.Static.SatelliteMass = df['Mass'].iloc[i]
                    ...
                    prop.ForceModel.Drag.AtmosphericDensityModel = atmDenEnu(dfstr['Density Model'].iloc[i])
                    if dfstr['SolarFluxFile'].iloc[i] == 'SolFlx_CSSI.dat':
                        dragFile.File = 'C:\\ProgramData\\AGI\\STK 11 (x64)\\DynamicEarthData\\' + SolFluxSigEnu(df['Flux Sigma Level'].iloc[i])
                    else:
                        dragFile.File = 'C:\\ProgramData\\AGI\\STK 11 (x64)\\DynamicEarthData\\' + dfstr['SolarFluxFile'].iloc[i]
                    ...
                    startEph = float(sat.Vgt.Events.Item('EphemerisStartTime').FindOccurrence().Epoch)
                    stopEph = float(sat.Vgt.Events.Item('EphemerisStopTime').FindOccurrence().Epoch)
                    HPOPLT = stopEph-startEph
                    df.loc[i,'HPOP Years'] = HPOPLT 

                    stop = time.time()

                    print('LifeTimeAnalysis'+str(processNum)+' '+str(i+1) + ' of ' + str(len(df)) + '. Total Time: '+ str(stop-start))
                    print('LifeTimeAnalysis'+str(processNum)+' HPOP Lifetime: '+ df.loc[i,'HPOP Years'].astype(str) + '. Runtime '+str(stopHPOP-startHPOP))
                    print(res.Item(0))

                    # Save as you go
                    df.to_csv(fileName) # Save the file for future use
                    print('\t')
                    print('LifeTimeAnalysis'+str(processNum) +' wrote to '+ fileName)
                    print('\t')
        ...
        if sc.Children.Contains(STKObjects.eSatellite,satName) == False:
            ...
            prop = sat2.Propagator.QueryInterface(STKObjects.IAgVePropagatorJ4Perturbation)
        else:
            sat = root.GetObjectFromPath('Satellite/'+satName)
            sat2 = sat.QueryInterface(STKObjects.IAgSatellite)
            prop = sat2.Propagator.QueryInterface(STKObjects.IAgVePropagatorJ4Perturbation)
        ...
        ii = 0
        prop.InitialState.Representation.AssignCartesian(STKUtil.eCoordinateSystemICRF,df['x'].iloc[ii],df['y'].iloc[ii],df['z'].iloc[ii],df['Vx'].iloc[ii],df['Vy'].iloc[ii],df['Vz'].iloc[ii])
        prop.Step = 1e-3
        epYr = epoch1
        prop.InitialState.OrbitEpoch.SetExplicitTime(epYr)
        ...
        cmd = ('SetLifetime */Satellite/' + satName +
               ' DensityModel "'+ dfstr['Density Model'].iloc[ii] + '"' +
               ' FluxSigmaLevel ' + dfstr['Flux Sigma Level'].iloc[ii] +
               ' OrbPerCalc ' + dfstr['Orb Per Calc'].iloc[ii] +
               ' GaussianQuad ' + dfstr['Gaussian Quad'].iloc[ii] +
               ' 2ndOrder ' + dfstr['2nd Order Oblateness'].iloc[ii])
        root.ExecuteCommand(cmd);
        cmd = ('SetLifetime */Satellite/' + satName +
                    ' SolarFluxFile "C:\\ProgramData\\AGI\\STK 11 (x64)\\DynamicEarthData\\' + dfstr['SolarFluxFile'].iloc[ii] + '"')
        root.ExecuteCommand(cmd)


        cmd = ('SetLifetime */Satellite/' + satName +
               ' DragCoeff ' + dfstr['Cd'].iloc[ii] +
               ' ReflectCoeff ' + dfstr['Cr'].iloc[ii] +
               ' DragArea ' + dfstr['Drag Area'].iloc[ii] +
               ' SunArea ' + dfstr['Sun Area'].iloc[ii] +
               ' Mass ' + dfstr['Mass'].iloc[ii])
        root.ExecuteCommand(cmd);
...
async def runTasks(executor, df, tradeStudy, showSTK=False, saveEveryNIter=15):
    ...
    for kk in range(tradeStudy.numCores):
        ...
        blocking_tasks.append(loop.run_in_executor(executor, RunSTK, dfSlice, tradeStudy, kk, showSTK, saveEveryNIter))
...
def loadSats(df,maxSats=100,maxDur=100):
    ...
    for ii in range(len(colors)):
        colorsInt.append(int('00'+"{0:#0{1}x}".format(colors.iloc[ii],4)[2:]+"{0:#0{1}x}".format(255-colors.iloc[ii],4)[2:],16))
    ...
    if len(df) <= maxSats:
        for ii in range(len(df)):
            ...
            graphics = sat2.Graphics.Attributes.QueryInterface(STKObjects.IAgVeGfxAttributesBasic)
            graphics.Color = colorsInt[ii]
           
            prop = sat2.SetPropagatorType(STKObjects.ePropagatorJ4Perturbation)
            prop = sat2.Propagator.QueryInterface(STKObjects.IAgVePropagatorJ4Perturbation)
            prop.UseScenarioAnalysisTime = False
            prop.InitialState.Epoch = root.ConversionUtility.ConvertDate('YYDDD','EpDay',str(df['epoch'].iloc[ii]))
            prop.InitialState.Representation.AssignCartesian(STKUtil.eCoordinateSystemICRF,df['x'].iloc[ii],df['y'].iloc[ii],df['z'].iloc[ii],df['Vx'].iloc[ii],df['Vy'].iloc[ii],df['Vz'].iloc[ii])
            prop.Step = 600
            prop.StopTime = 1
            prop.Propagate()
    else:
        print('Number of satellites is too large, ' + str(len(df)) + '. Please reduce the number of satellites to beneath '+str(maxSats))
...
def readResults(tradeStudy):
    ...
    fileCount = 0
    for kk in range(tradeStudy.numCores):
        fileNamekk = str.split(fileName,'.')[0] + str(kk) + '.' + str.split(fileName,'.')[-1]
...
def mergeResults(tradeStudy):
    fileName = os.getcwd()+'\\Results\\'+tradeStudy.fileName
    for kk in range(tradeStudy.numCores):
        fileNamekk = str.split(tradeStudy.fileName,'.')[0] + str(kk) + '.' + str.split(tradeStudy.fileName,'.')[-1]
        fileNamekk = os.getcwd()+'\\Results\\'+fileNamekk
        dfTemp = pd.read_csv(fileNamekk,index_col=0) # Read previously run Lifetime results
        if kk == 0:
...
def csvToDf(fileName):
    ...
    dfRes = pd.read_csv(fileName,index_col=0).sort_values('Run ID').reset_index(drop=True) 
    cols = [col for col in dfRes.columns if col not in ['SolarFluxFile','Density Model','2nd Order Oblateness']]
...
def saveTradeStudy(tradeStudy):
    tradeStudyFileName = os.getcwd()+'\\TradeStudyFiles\\'+'tradeStudy'+tradeStudy.fileName.split('.')[0]+'.pkl'
