...
volumeDataProvider = volumetric.DataProviders.GetDataPrvTimeVarFromPath("Satisfaction Volume")
...
for time in analysisTimes:
    ...
    valueDataProvider = volumetric.DataProviders.GetDataPrvFixedFromPath("Volumetric Values at Time")
    # Pre data time must be UTCG string
    valueDataProvider.PreData = root.ConversionUtility.ConvertDate('EpSec', 'UTCG', str(time))
    ...
    satisfiedPoints = [float(point) for point in data if float(point) >= satisfactionValue]
