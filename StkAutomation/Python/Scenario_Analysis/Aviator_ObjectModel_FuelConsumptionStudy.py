...
def AviatorParametricDemo(stkRoot):
    aircraft = AgAircraft(
        stkRoot.CurrentScenario.Children.New(AgESTKObjectType.eAircraft, "AvtrAircraft")
    )
    ...
    advTool.PowerplantStrategy = (
        AgEAvtrAdvFixedWingPowerplantStrategy.eTurbofanHighBypass
    )
    ...
    enroute = procedures.Add(
        AgEAvtrSiteType.eSiteRunway, AgEAvtrProcedureType.eProcEnroute
    )
    ...
    for altitude in altitudes:
        ...
        avtrProp.Propagate()
        flightDP = AgDataPrvTimeVar(
            aircraft.DataProviders["Flight Profile By Time"]
        ).Exec(4.5, 6.5, 3600)
