...
def addNodeConstraints(stkRoot,nodeConstraints,timesEdgesDistancesDelaysBandwidths,start,stop,step):
...
def computeDataTransferThroughNetwork(start,stop,step,timesEdgesDistancesDelaysBandwidths,timesDataToTransfer,timesDataTransferred,startingNodes,endingNodes,priorityCase=1,nodeTransferPriority=[],nodeDestinationPriority=[],metric='distance',maxDataTransfer=1e15,breakOnceAllDataIsTransferred = True,overrideData=False,printTime=False,filename='',minPacketSize = 0):
    ...
    if needToCompute == True:
        ...
        #     ii = len(uniqueBandwidths)-1
            ...
            if priorityCase in [2,3,4]:
                layers = computePriorityNetworkLayers(priorityCase,tDataToTransfer,tDataTransferred)
            ...
            for layer in range(numLayers):
                ...
                while totalBandwidthTransferred < maxDataTransfer and pathsAvailable: # see if there are more layers to add
                    ...
                    if any([node in G.nodes() for node in startingNodes]) and any([node in G.nodes() for node in endingNodes]):
                        ...
                        if not np.isnan(float(metricVal)):
                            ...
                            bandwidths.append(maxDataTransfer-totalBandwidthTransferred) # Don't go past maximum network transfer
                            dataTransferred = min(bandwidths)

                            # Handle minimum packet size, pop off edges with too little remaining bandwidth and repeat
                            transferData = True
                            if (dataTransferred - minPacketSize) < -1e-6:  # Allow for some numerical issues, effectively less than 0
                                for edge in edges:
                            ...
                            if (dataTransferred - minPacketSize) >= -1e-6 or transferData:
                                ...
                                for edge in edges:
                                    ...
                                    timesEdgesDistancesDelaysBandwidths[t].update({edge:(distance,timeDelay,availableBandwidth-dataTransferred)})
                                    if (timesEdgesDistancesDelaysBandwidths[t][edge][2]) < 1e-6: # Allow for some numerical issues, effectively 0
                                        timesEdgesDistancesDelaysBandwidths[t].pop(edge)

                                # Update data transfer
                                tDataToTransfer.update({startNode:tDataToTransfer[startNode]-dataTransferred})
                                tDataTransferred.update({endNode:tDataTransferred[endNode]+dataTransferred})
                        else:
                            pathsAvailable = False

                        if dataTransferredFlag:
                            if (totalBandwidthTransferred-dataTransferred) > 0: # check if data has already been transferred
                                if not np.isnan(float(metricVal)): # don't append nan values if data has already been transferred/paths have been found
                                    timeStrandMetric.append((t,strandShort,metricVal,dataTransferred))
                            else:
                                    timeStrandMetric.append((t,strandShort,metricVal,dataTransferred))
                    else:
                        pathsAvailable = False
            ...
            if t != times[-1]:
                ...
                for node in timesDataTransferred[t].keys():
                    timesDataTransferred[times[ii+1]].update({node:tDataTransferred[node]+timesDataTransferred[times[ii+1]][node]})
        ...
        df = pd.DataFrame(timeStrandMetric,columns=['time','strand',metric,'data transferred'])
        ...
        dfTimesDataTransferred = convertTimesDataToDataFrame(timesDataTransferred,simulationStopTime)
        ...
        return df,dfTimesDataToTransfer,dfTimesDataTransferred,timesEdgesDistancesDelaysBandwidths,simulationStopTime
...
def createDirectedChains(stkRoot,constellationOrderLists,start=[],stop=[],color=Colors.FromRGB(0, 196, 196)):
    chainNames  = []
...
def buildChains(stkRoot,constellationOrder,start=[],stop=[],color=Colors.FromRGB(0, 196, 196)):
    chainNames  = []
    for ii in range(len(constellationOrder)-1):
        ...
        if stkRoot.CurrentScenario.Children.Contains(AgESTKObjectType.eChain,chainName):
            ...
            chain = AgChain(stkRoot.CurrentScenario.Children.New(AgESTKObjectType.eChain,chainName))
...
def turnGraphicsOnOff(stkRoot,objPaths,onOrOff = 'On',parentsOnly = False):
    ...
    for objPath in objPaths:
        ...
        stkRoot.ExecuteCommand(f'Graphics */{parentPath} Show {onOrOff}') # Turn parent object on first
...
def getNodesFromConstellation(stkRoot,constellationName,overrideData=False):
    ...
    # Get nodes from constellation
        ...
        constellation = AgConstellation(stkRoot.GetObjectFromPath(f'Constellation/{constellationName}'))
        nodes = []
...
def getStrands(stkRoot,chainName,start,stop,overrideData=False,printLoadedMessage=True):
    ...
    # Compute and parse strands
        ...
        chain = stkRoot.GetObjectFromPath(f'Chain/{chainName}')
        strandAccessDataProvider = AgDataPrvInterval(chain.DataProviders.Item('Strand Access'))
...
def createChains(stkRoot,startingConstellation,firstConnectingConstellation,endingConstellation,secondConnectingConstellation='',color=Colors.FromRGB(0, 196, 196)):
    conStart = stkRoot.GetObjectFromPath(f'*/Constellation/{startingConstellation}')
    conFirst = stkRoot.GetObjectFromPath(f'*/Constellation/{firstConnectingConstellation}')
    ...
    if secondConnectingConstellation != '':
        conSecond = stkRoot.GetObjectFromPath(f'*/Constellation/{secondConnectingConstellation}')
...
def filterObjectsByType(stkRoot, objectType, parentObject, filterName):
    ...
    for child in parentObject.Children:
        ...
        listOfObjects.extend(filterObjectsByType(stkRoot, objectType, child, filterName))
