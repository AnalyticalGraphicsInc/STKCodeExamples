...
def createPointsFromCovariance(root,satName,timeStep=60,sigmaLevel=3,createPoints=False,createSats=False):
    ...
    elems = ['Time','Major Sigma','Intermediate Sigma','Minor Sigma','Major Sigma Dir x','Major Sigma Dir y','Major Sigma Dir z','Minor Sigma Dir x','Minor Sigma Dir y','Minor Sigma Dir z']
    ...
    intermediateDir = np.asarray([np.cross(minorDir[ii],majorDir[ii]) for ii in range(len(covData[:]))])
    scaledMajorDir = majorDir*majorSigma.reshape(len(majorSigma),1)
    scaledIntermediateDir = intermediateDir*intermediateSigma.reshape(len(intermediateSigma),1)
    ...
    ephFiles = [majorPos,majorNeg,intermediatePos,intermediateNeg,minorPos,minorNeg]
    names = ['majorPos.e','majorNeg.e','intermediatePos.e','intermediateNeg.e','minorPos.e','minorNeg.e']
    ...
    for ii,ephFile in enumerate(ephFiles):
        with open(scenarioPath+names[ii],'w') as f:
            ...
            for jj,line in enumerate(majorPos):
                f.write("   {} {} {} {}\n".format(times[jj],ephFile[jj,0],ephFile[jj,1],ephFile[jj,2]))
    ...
    if createPoints:
        ...
        for ii,name in enumerate(names):
            ...
            pointsFactory.Create(name,'Point On the Covariance',AgECrdnPointType.eCrdnPointTypeFile)
            point = sat.Vgt.Points.Item(name)
            point.Filename = scenarioPath+name+'.e'
            sat.VO.Vector.RefCrdns.Add(AgEGeometricElemType.ePointElem,'Satellite/{} {} Point'.format(satName,point.Name)) # Add to VO
    ...
    if createSats:
        ...
        for ii,name in enumerate(names):
            ...
            # Clear graphics
            satii.VO.Pass.TrackData.PassData.Orbit.SetLeadDataType(AgELeadTrailData.eDataNone)
            satii.VO.Pass.TrackData.PassData.Orbit.SetTrailDataType(AgELeadTrailData.eDataNone)
...
createPointsFromCovariance(root,satName,timeStep=60,sigmaLevel=sigmaLevel,createPoints=True,createSats=True)
