...
def CreateConstellation(root, txt, txtBox, ssc=00000, howToCreate="satsinstk"):
    ...
    if howToCreate == "code":
        ...
        fid.close()
        txtBox.insert(
            END, "Created: " + txt.get().replace(" ", "-").replace(" ", "-") + ".tce\n"
        )
...
def runDeckAccess(
    root, startTime, stopTime, comboCon, comboDA, txtBox, constraintSatName=""
):
    ...
    if accessObjPath == "":
        ...
        if root.CurrentScenario.Children.Contains(
            STKObjects.eSatellite, constraintSatName
        ):
            cmd = (
                "DeckAccess */"
                + accessObjPath
        ...
        txtBox.insert(
            END,
            accessObjPath.split("/")[-1] + ": " + str(NumOfSC) + " sats w/ access\n",
        )
...
def LoadSatsFromFileUsingTemplate(
    root, startTime, stopTime, comboCon, selected, txtBox, satTempName, color="cyan"
):
    startTime = root.ConversionUtility.ConvertDate("UTCG", "EpSec", str(startTime))
    stopTime = root.ConversionUtility.ConvertDate("UTCG", "EpSec", str(stopTime))
    TLEFileName = cwdFiles + "\\Constellations\\" + comboCon.get() + ".tce"
    ...
    LoadSatsUsingTemplate(
        root, dfLoad, startTime, stopTime, TLEFileName, satTempName, color=color
    )
