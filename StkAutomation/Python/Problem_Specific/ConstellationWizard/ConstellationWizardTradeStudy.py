...
constellationList = [
    ...
    "SpaceX",
    "Telesat",
]  # List of different constellations to run analysis
satTemplateList = [
    "OneWeb",
    "StarLink1",
    "TeleSat1",
]  # Used for deck access constraints and child objects when loading satellites. This can be an empty string ''
...
startTimes = np.arange(
    0, 3600, 600
)  # Deck Access and analysis start time. Relative to the scenario start time [EpSec]
stopTimes = np.arange(
    600, 4200, 600
...
for constellationName, satTemplateName, transmitters in zip(
    ...
    for startTime, stopTime in zip(startTimes, stopTimes):
        ...
        NumOfSC, deckAccessFileName, deckAccessTLEFileName = runDeckAccess(
            root,
            startTime,
            stopTime,
            TLEFileName,
            accessObjPath,
            constraintSatName=satTemplateName,
        )
        ...
        dfToTLE(
            dfTLEwAccess, NewTLEFileName
        )  # Example of how to save DeckAccess satellites to a TCE file
        ...
        dfChainData = chainAnalysis(
            root, chainPath, objsToAdd, startTime, stopTime, exportFileName
        )
        ...
        dfCovAnalysis = covAnalysis(
            root, covDefPath, coverageAssets, startTime, stopTime, exportFileName
        )
        ...
        exportFileName = (
            ...
            + "\\AnalysisResults\\"
            + constellationName
            + "InterferanceLinkBudgetStart"
            + str(int(startTime))
            + "Stop"
            + str(int(stopTime))
            + ".csv"
        )
        dfCommSys = commSysAnalysis(
            root,
            commSysPath,
            accessReceiver,
            interferanceTransmitters,
            startTime,
            stopTime,
            exportFileName,
        )
