...
class satellite:
    def __createSat(self):
        self.sat = AgSatellite(
            scenario.Children.New(AgESTKObjectType.eSatellite, self.name)
        )
    ...
    def fromTLE(self, SSCNum, FileName):  # Create a satellite from a TLE
        self.reference = AgSatellite(
            scenario.Children.New(AgESTKObjectType.eSatellite, "ref" + self.name)
        )  # Make a reference satellite
        ...
        follow = self.driver.MainSequence.Insert(
            3, "Follow", "-"
        )  # Follow the reference satellite
        follow.Leader.BindTo("Satellite/ref" + self.name)
        StopDuration = follow.SeparationConditions.Item(0)
        TripVal = 0
        AgVAStoppingCondition(
            StopDuration.Properties
        ).Trip = TripVal  # Make the satellite separate at the beginning
        self.driver.RunMCS()
        self.sat.VO.Pass.TrackData.PassData.Orbit.SetLeadDataType(
            7
        )  # Change lead and trail paths to be time based
    ...
    def updateEphemerisReference(
        self, fileName
    ):  # Update a satellite's reference by exporting ephemeris
    ...
    def fromReference(self, inTrack, duration, epoch, targetsat):
        ...
        VbarRef.ScriptingTool.Parameters.Item("Desired_InTrack").ParamValue = str(
            inTrack
        )  # Desired_InTrack
        VbarRef.ScriptingTool.Parameters.Item("Ephem_Duration").ParamValue = str(
            duration
        )  # Ephem_Duration
        VbarRef.ScriptingTool.Parameters.Item(
            "VBar_Epoch"
        ).ParamValue = epoch  # VBar_Epoch
        VbarRef.ScriptingTool.Parameters.Item(
            "Target_Satellite"
        ).ParamValue = targetsat  # Target_Satellite
    ...
    def setRef(self, targetsat):
        ...
        SetReference.ScriptingTool.Parameters.Item("Reference Vehicle").ParamValue = (
            "ref" + targetsat.name
        )
        self.VVLH(targetsat)
    ...
    def GEOtoGEORendezvous(self, targetsat, departure, drift):
        ...
        GeoRendez.ScriptingTool.Parameters.Item("Lead_Satellite").ParamValue = (
            "ref" + self.name
        )
        GeoRendez.ScriptingTool.Parameters.Item("Target_Satellite").ParamValue = (
            "ref" + targetsat.name
        )
        GeoRendez.ScriptingTool.Parameters.Item("Desired_DriftRate").ParamValue = str(
            drift
        )
        GeoRendez.ScriptingTool.Parameters.Item(
            "Earliest_Departure_Time"
        ).ParamValue = departure
    ...
    def coast(self, TripVal, method):
        ...
        if method == "Duration":
            Coast.ScriptingTool.Parameters.Item("Coast_Duration").ParamValue = str(
                TripVal
            )
        elif method == "Revs":
    ...
    def NMC(
        ...
        NMCircum.ScriptingTool.Parameters.Item(
            "RBar_CrossTrack_Offset"
        ).ParamValue = RBarCross
        NMCircum.ScriptingTool.Parameters.Item(
            "VBar_CrossTrack_Offset"
        ).ParamValue = VBarCross
        NMCircum.ScriptingTool.Parameters.Item(
            "Init_Phase_Angle"
        ).ParamValue = initPhaseAng
        NMCircum.ScriptingTool.Parameters.Item("Transfer_Duration").ParamValue = str(
            duration
        )
        NMCircum.ScriptingTool.Parameters.Item(
            "InTrack_Offset"
        ).ParamValue = inTrackOffset
        NMCircum.ScriptingTool.Parameters.Item(
            "InTrack_Drift"
        ).ParamValue = inTrackDrift
    ...
    def VBarHop(self, offset):
        ...
        VBarHop.ScriptingTool.Parameters.Item("Final_VBar_Offset").ParamValue = str(
            offset
        )
    ...
    def GEOtoGEORendezvousNoLead(self, targetsat, drift):
        self.setRef(targetsat)
        driver = self.sat.Propagator
        GeoRendez = driver.MainSequence.InsertByName(
        ...
        for maneuver in intProps:
            if maneuver[0]:
                ...
                if maneuver[1] == "Set Reference":
                    ...
                    maneuveringSat.GEOtoGEORendezvous(
                        targetSat, maneuver[3], float(maneuver[4]) / days
                    )
                elif maneuver[1] == "GEO to GEO Rendezvous NoLead":
                    maneuveringSat.GEOtoGEORendezvousNoLead(
                        targetSat, float(maneuver[4]) / days
                    )
                elif maneuver[1] == "NMC":
                    maneuveringSat.NMC(
                        maneuver[6],
        ...
        for maneuver in defProps:
            if maneuver[0]:
                ...
                if maneuver[1] == "Set Reference":
                    ...
                    maneuveringSat.GEOtoGEORendezvous(
                        targetSat, maneuver[3], int(maneuver[4]) / days
                    )
                elif maneuver[1] == "GEO to GEO Rendezvous NoLead":
                    maneuveringSat.GEOtoGEORendezvousNoLead(
                        targetSat, int(maneuver[4]) / days
                    )
                elif maneuver[1] == "NMC":
                    maneuveringSat.NMC(
                        maneuver[6],
...
computeManeuverButton = Button(
    maneuverMenuFrame, text="Add to MCS", command=addMCSSegments
)
