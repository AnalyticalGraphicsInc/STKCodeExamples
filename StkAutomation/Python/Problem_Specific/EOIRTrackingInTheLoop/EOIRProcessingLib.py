...
def getSensorFOVAndPixels(sensor):
    ...
    elems = [
        "Horizontal pixels",
        "Vertical pixels",
        "Horizontal geometric FOV",
        "Vertical geometric FOV",
    ]
...
def computeSensorBodyToParentRotations(sensor, tstart, tstop, tstep, axes=""):
    if not axes:
        dataProvider = sensor.DataProviders.Item("Body Axes Orientation").Group.Item(
            "Parent Body Axes"
        )
        ...
        if len(data) == 0:
            print(
                "Transformation could not be completed. Check the sensor point is defined at this time"
            )
...
def updatePointingDir(azError, elError, timeRotationZYX):
    ...
    r = R.from_euler(
        "zyz",
        [timeRotationZYX[3], timeRotationZYX[2], timeRotationZYX[1]],
        degrees=True,
    )
...
def RADECToMeasurementFileLine(
    root, t, ra, dec, targetID=1001, sensorID=1000, RAstd=6.0, DECstd=6.0
):

    # Convert time
    yyddd, fractionOfDay = root.ConversionUtility.ConvertDate(
        "EpSec", "YYDDD", str(t)
    ).split(".")
    year = int(yyddd[0:2])
    ...
    line = "{:7d}1203     {:0>2d}{:0>3d}{:0>5d}{:0<6d}0{}{} {:>2d}{:0>2d}{:06.3f}{}{:0>2d}{:0>2d}{:05.2f}{}0{}{:5.2f}{:5.2f}   {:5d}\n".format(
        ...
        DECmm,
        DECsec,
        annualAberrationApplied,
        diurnalAberrationApplied,
        RAstd,
        DECstd,
        sensorID,
    ).replace(
    ...
    for offset in range(0, 1):
        ...
        for ii in np.arange(0 + offset, X, 4):
            for jj in np.arange(0 + offset, Y, 4):
                ...
                every4thPixel[int((ii - offset) / 4), int((jj - offset) / 4)] = meanpic[
                    ii, jj
                ]
...
def sortBySNR(image, objectCenters):
    ...
    objectSNRS = np.array(
        [
            image[objectCenter[0], objectCenter[1]] / standardDev
...
def getObjectCenters(
    image, method="localpeaks", minSNR=10, percentofmax=0.25, maxObjects=4
):
    ...
    if getMaxSNR(image) >= minSNR:
        if method.lower() == "localpeaks":
            objectCenters = peak_local_max(
                image, min_distance=1, threshold_rel=percentofmax, num_peaks=maxObjects
            )
        else:
            ...
            if method.lower() == "kmeans":
                ...
                groupedImage = image
                std = np.std(
                    groupedImage.reshape(-1, 1)
                )  # pixel values to variance, signal/variance
            ...
            points = np.array(
                [
                    ...
                    if groupedImage[x, y] == objectColor
                ]
            )
...
def findNumberOfObjects(points, maxObjects=4):
    ...
    if scoresInertia[1] - scoresInertia[0] > 0:
        objectCount = 1
        print(
            "Adding more clusters made the inertia score worse, setting the object count to 1"
        )
...
def writeSensorPointingFile(
    sensorPointingHistory, fileName="SensorPointing.sp", axes=""
):
...
def createVideo(sensorName, times, tstep, imageFolder, realTimeRate=1):
    ...
    images = [
        "{}\\{}Time{}Tagged.png".format(
            imageFolder, sensorName, str(float(t)).replace(".", "_")
        )
        for t in times
    ]
...
def getState(ephemerisFile, tPlus1):
    ...
    state = np.array([splitVal for splitVal in splitVals if splitVal != ""]).astype(
        float
    )
