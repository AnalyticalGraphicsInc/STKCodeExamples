...
class Strand:
    ...
    def __init__(self, fromObject: IAgStkObject, toObject: IAgStkObject, intervals: list[AccessInterval]):
    ...
    def writeIntervalFile(self, filepath: str, delay: float = 0):
        ...
        with open(filepath, 'w') as file:
            ...
            for interval in self.intervals:
                startTime = ROOT.ConversionUtility.ConvertDate(interval.unit, 'UTCG', str(interval.start))
                stopTime = ROOT.ConversionUtility.ConvertDate(interval.unit, 'UTCG', str(interval.stop + delay))
    ...
    def removeConstraints(self, stkObject: int, preserveObjects: bool):
        ...
        for obj in objects:
            ...
            obj.AccessConstraints.RemoveConstraint(AgEAccessConstraints.eCstrLineOfSight)

            try:
                obj.AccessConstraints.RemoveConstraint(AgEAccessConstraints.eCstrFieldOfView)
            except:
    ...
    def applyIntervalFile(self, filename: str, stkObject: IAgStkObject):
        ...
        '''
        intervalConstraint: AgAccessCnstrIntervals = stkObject.AccessConstraints.AddConstraint(AgEAccessConstraints.eCstrIntervals)
...
def getAccessIntervals(chain: AgChain) -> list[Strand]:
    ...
    strandNames = strandAccessResults.DataSets.GetDataSetByName('Strand Name (Long)').GetValues()
    ...
    for i, strandName in enumerate(strandNames):
        ...
        if not strandExists:
            strands.append(
                Strand(object1, object2, [interval])
            )
...
def extendAccesses(strand: Strand, preserveObjects: bool):
    ...
    filepath = os.path.join(scenarioDirectory, f'{strand.fromObject.InstanceName}-to-{strand.toObject.InstanceName}Intervals.int')
