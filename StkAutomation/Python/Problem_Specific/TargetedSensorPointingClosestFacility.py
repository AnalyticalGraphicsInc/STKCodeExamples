...
def computeRangeSatsToPlaces(satList: list, placeNames: list):
    ...
    for sat in satList:
        ...
        for placeName in placeNames:
            elems = sat.DataProviders.GetDataPrvTimeVarFromPath(
                "Vectors(Inertial)/" + placeName
            ).ExecElements(
                scenario.StartTime,
                scenario.StopTime,
                algTimeStep,
...
def setPointingForEachSensor(
    satList: list, placeList: list, placeNames: list, sensList: list, ranges, times
):
    ...
    for sat in satList:
        ...
        for rangeSet in rangesForSat:
            ...
            for placeIndex in smallToLargeInd:
                if (
                    allAccesses[i][placeIndex]
                    .Vgt.EventIntervalLists.Item("AccessIntervals")
                    .Occurred(epSec)
                ):
        ...
        for y in range(len(placeNames)):
            ...
            for j in range(len(pointingTimesForPlaceForSat[y])):
                if j == (len(pointingTimesForPlaceForSat[y]) - 1):
                    ...
                    pointingTimesForPlaceForSat[y][j + 1]
                    - pointingTimesForPlaceForSat[y][j]
                    == algTimeStep
                ):
                    if newPass:
                        intervalsForPlace.append(pointingTimesForPlaceForSat[y][j])
            ...
            intervals = (
                AgSatellite(sat)
            ).Vgt.EventIntervalLists.Factory.CreateEventIntervalListFixed(
                "PointingTo" + placeNames[y],
                "Interval for when this satellite will point the sensor at "
                + placeNames[y],
            )
