...
cmd = 'VO ' + sensorPath + ' Obscuration CentralBody Sun On DontRenderNormalWindows On DeleteData'
root.ExecuteCommand(cmd)
cmd = 'VO ' + sensorPath + ' Obscuration Compute "' + scenario.StartTime + '" "' + scenario.StopTime + '" ' + str(step)
root.ExecuteCommand(cmd)

# Pull times and percent obscuration
sensor = root.GetObjectFromPath(sensorPath)
obsResult = AgDrResult(sensor.DataProviders.Item('Obscuration').ExecElements(scenario.StartTime, scenario.StopTime, step, ['Time', 'Percent Obscured']))
...
try:
    obsScalar = AgCrdnCalcScalarFile(sensor.Vgt.CalcScalars.Factory.CreateCalcScalarFile('sunObscuration', '', saveDir))
except:
...
try:
    obsCondition = AgCrdnConditionScalarBounds(sensor.Vgt.Conditions.Factory.CreateConditionScalarBounds('ifSunObscured', ''))
except:
    obsCondition = AgCrdnConditionScalarBounds(sensor.Vgt.Conditions.Item('ifSunObscured'))
obsCondition.Scalar = obsScalar
obsCondition.Operation = AgECrdnConditionThresholdOption.eCrdnConditionThresholdOptionAboveMin
...
try:
    obsIntervals = AgCrdnEventIntervalListCondition(sensor.Vgt.EventIntervalLists.Factory.CreateEventIntervalListCondition('sunObscurationTimes', ''))
except:
    obsIntervals = AgCrdnEventIntervalListCondition(sensor.Vgt.EventIntervalLists.Item('sunObscurationTimes'))
...
try:
    sunVec = sensor.Vgt.Vectors.Factory.CreateDisplacementVector('toSun', sensorCenter, sunCenter)
except:
