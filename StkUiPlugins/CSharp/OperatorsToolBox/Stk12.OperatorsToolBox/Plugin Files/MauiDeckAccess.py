...
if len(satName) == 0:
    cmd = 'DeckAccess */' + objPath +' "' + startTime + '" "'+ stopTime +'" Satellite "' + tleFilepath+ '" SortObj OutFile "'+ deckAccessFile+'"'
else:
    print('Using Satellite Constraint Object')
    cmd = 'DeckAccess *' + objPath + ' "' + startTime + '" "'+ stopTime +'" Satellite "' + tleFilepath+ '" SortObj OutFile "'+ deckAccessFile+'" ConstraintObject */Satellite/'+satName
...
MTOName = LoadMTO(root,deckAccessTLE,timestep=30,color='cyan',orbitsOnOrOff='off',orbitFrame='Inertial')
generatePlots = True
